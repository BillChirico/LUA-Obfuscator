			...
			<main className="absolute inset-0 z-10 flex flex-col p-4 sm:p-6 gap-4 lg:gap-6">
				...
				<header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
					<div className="flex items-center gap-3">
						<div
							className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-[#007AFF] to-[#5856D6] flex items-center justify-center shadow-lg"
							aria-hidden="true"
						>
							<Lock className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
						</div>
						<div>
							<h1 className="text-xl sm:text-2xl font-bold text-white">Lua Obfuscator</h1>
							<p className="text-xs sm:text-sm text-gray-300 hidden sm:block">Professional Lua code protection</p>
						</div>
					</div>
					<nav className="flex flex-wrap gap-2 w-full sm:w-auto" aria-label="Main actions">
						<Button
							onClick={copyToClipboard}
							disabled={!outputCode}
							className="bg-white/10 hover:bg-white/20 text-white border border-white/20 flex-1 sm:flex-none"
						...
						<Button
							...
							className="bg-gradient-to-r from-[#007AFF] to-[#5856D6] hover:from-[#0066CC] hover:to-[#4644C7] text-white shadow-lg flex-1 sm:flex-none"
				...
				{/* Main Content */}
				<section
					className="flex-1 flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-6 min-h-0 overflow-y-auto"
				...
				>
					...
					<div className="lg:col-span-8 flex flex-col lg:grid lg:grid-cols-2 gap-4 lg:min-h-0">
						{/* Input Editor */}
						<section aria-labelledby="input-code-heading" className="flex flex-col h-[300px] lg:h-auto lg:min-h-0">
							<Card className="flex-1 bg-background/40 backdrop-blur-xl border-white/10 overflow-hidden flex flex-col h-full p-0 gap-0">
								<div className="p-3 sm:p-4 border-b border-white/10">
									<div className="flex items-center gap-2">
										<Code className="w-4 h-4 text-[#007AFF]" aria-hidden="true" />
										<h2 id="input-code-heading" className="text-sm font-semibold text-white">
											Original Lua Code
										</h2>
									</div>
								</div>
						...
						{/* Output Editor */}
						<section aria-labelledby="output-code-heading" className="flex flex-col h-[300px] lg:h-auto lg:min-h-0">
							<Card className="flex-1 bg-background/40 backdrop-blur-xl border-white/10 overflow-hidden flex flex-col h-full p-0 gap-0">
								<div className="p-3 sm:p-4 border-b border-white/10">
									<div className="flex items-center gap-2">
										<Lock className="w-4 h-4 text-[#5856D6]" aria-hidden="true" />
										<h2 id="output-code-heading" className="text-sm font-semibold text-white">
											Obfuscated Output
										</h2>
									</div>
								</div>
					...
					<aside className="lg:col-span-4 lg:overflow-auto" aria-labelledby="settings-heading">
						<Card className="bg-background/40 backdrop-blur-xl border-white/10 p-4 sm:p-6">
							<div className="flex items-center gap-2 mb-4 sm:mb-6">
								<Settings className="w-4 h-4 sm:w-5 sm:h-5 text-[#007AFF]" aria-hidden="true" />
								<h2 id="settings-heading" className="text-base sm:text-lg font-semibold text-white">
							...
							<div className="space-y-6">
								...
								<div className="space-y-4">
									...
									<p className="text-xs text-gray-400 -mt-2">Convert strings to byte arrays using string.char()</p>
									...
									<p className="text-xs text-gray-400 -mt-2">Remove comments and whitespace</p>
								</div>
								...
								<div className="space-y-3 pt-4 border-t border-white/10">
									...
									<div className="text-xs text-gray-400">
										{settings.compressionLevel === 0 && <p>No automatic obfuscation enabled</p>}
										{settings.compressionLevel >= 10 && settings.compressionLevel < 20 && (
											<p>
												<strong className="text-blue-300">Active:</strong> Minify
											</p>
										)}
										{settings.compressionLevel >= 20 && settings.compressionLevel < 40 && (
											<p>
												<strong className="text-blue-300">Active:</strong> Minify, Mangle Names
											</p>
										)}
										{settings.compressionLevel >= 40 && settings.compressionLevel < 60 && (
											<p>
												<strong className="text-blue-300">Active:</strong> Minify, Mangle Names, Encode Strings
											</p>
										)}
										{settings.compressionLevel >= 60 && settings.compressionLevel < 80 && (
											<p>
												<strong className="text-blue-300">Active:</strong> Minify, Mangle Names, Encode Strings, Encode
												Numbers ({settings.compressionLevel}%)
											</p>
										)}
										{settings.compressionLevel >= 80 && (
											<p>
												<strong className="text-blue-300">Active:</strong> All techniques enabled (
												{settings.compressionLevel}% intensity)
											</p>
										)}
									</div>
								</div>
								...
								<div className="pt-4 border-t border-white/10">
									<div className="bg-[#007AFF]/10 border border-[#007AFF]/20 rounded-lg p-4">
										<p className="text-xs text-blue-200">
											<strong>💡 Tip:</strong> Use the Protection Level slider for quick presets, or manually toggle
											individual techniques for fine-grained control. Higher protection levels provide stronger
											obfuscation but may impact performance.
										</p>
									</div>
								</div>
							</div>
